...
import { getVaults, removeVault, addTransaction, calculateGrowth, type Vault } from "@/app/lib/storage";
...
export default function WithdrawPage() {
  ...
  const handleWithdraw = () => {
    ...
    addTransaction({
      type: "Withdraw",
      strategyName: selectedVault.strategyName,
    ...
    // If withdrawing full amount, remove vault
    if (numericAmount >= maxBalance * 0.99) {
      removeVault(selectedVault.id);
    }

    router.push("/execute?action=withdraw");
  };
      ...
      <div className="min-h-screen flex flex-col items-center overflow-hidden relative">
        ...
        <main className="flex-1 flex flex-col items-center justify-center w-full max-w-2xl px-6 relative z-10 pt-20">
          <header className="text-center mb-12">
            <h1 className="font-headline text-5xl font-extrabold tracking-tighter mb-4 text-white">Withdraw Funds</h1>
          ...
          <div className="w-full bg-surface-container-low rounded-[2rem] p-8 shadow-[0_0_64px_rgba(255,110,132,0.04)] relative overflow-hidden">
            ...
            <div className="flex flex-col gap-4 mb-10 relative">
              <label className="font-label text-xs uppercase tracking-widest text-on-surface-variant font-bold">From Strategy</label>
              <button
                onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                className="w-full p-5 rounded-xl bg-surface-variant border border-outline-variant/20 hover:border-outline-variant/50 transition-colors flex items-center justify-between"
              >
                <div className="text-left">
                  <div className="font-headline font-bold text-white text-lg capitalize">{selectedVault?.strategyName || "Select Vault"}</div>
                  <div className="font-label text-xs text-on-surface-variant capitalize">{selectedVault?.risk || ""} risk · {selectedVault?.steps.length || 0} steps</div>
                </div>
                <div className="flex items-center gap-4 text-right">
                  <div>
                    <div className="font-bold text-white">{maxBalance.toFixed(4)} SOL</div>
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-full bg-surface-container-high rounded-xl border border-outline-variant/20 shadow-2xl z-20 overflow-hidden">
                  {vaults.map((vault) => {
                    ...
                    return (
                      ...
                      >
                        <span className="font-bold text-white capitalize">{vault.strategyName}</span>
            ...
            <div className="relative flex flex-col items-center py-6 border-b border-outline-variant/10">
              ...
              />
              <div className="mt-4 font-body text-on-surface-variant text-sm font-bold">SOL</div>
            </div>
            ...
            {numericAmount > 0 && (
              <div className="mt-8 bg-surface-container-highest/50 rounded-xl p-5 space-y-3 border border-outline-variant/10">
                <div className="flex justify-between text-sm">
                  <span className="text-on-surface-variant flex items-center gap-2"><IconBolt size={14}/> Network Fees (Est.)</span>
                  <span className="text-white">{estGas.toFixed(4)} SOL</span>
                </div>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-tertiary font-bold">{receiveAmount > 0 ? receiveAmount.toFixed(4) : "0.0000"} SOL</span>
                </div>
              </div>
            )}
          </div>
